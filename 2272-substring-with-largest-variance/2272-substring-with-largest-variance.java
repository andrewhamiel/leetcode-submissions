    ...
    public int largestVariance(String s) {
        ...
        for (int charA = 0; charA < 26; charA++){
            for (int charB = 0; charB < 26; charB++){
                int remainA = frequency[charA], remainB = frequency[charB];
                ...
                for (int k = 0; k < s.length(); k++) {
                    ...
                    if (currChar == charB) charBFreq++;                    // Increment frequency for charB
                    ...
                    if (charAFreq > 0) max = Math.max(max, charBFreq - charAFreq);
