...
class Trie{
    ...
    public void add(String word){
        ...
        for (char ch : word.toCharArray()){
            int index = ch-'a';
            if (temp.hash[index] == null) temp.hash[index] = new TrieNode();
    ...
    private boolean dfs(TrieNode node, char[] arr, int index, int dfsCount){
    	 ...
    	 * */
        if (index == arr.length) return node.isEnd && dfsCount > 1;
        //if current node is the end of some word
        if (node.isEnd) {
        	/**
             ...
             * word is introduced
             * */
            if (node.hash[arr[index]-'a'] != null) result |= dfs(node.hash[arr[index]-'a'],arr,index+1,dfsCount);
            /**
             * result will be the OR of both the cases
             * */
            return result;
        }
        if (node.hash[arr[index]-'a'] == null) return false; //if the current character is not present at at curr node
