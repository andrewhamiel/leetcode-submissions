class Codec {
    ...
    private void _serializeHelper(Node root, StringBuilder sb) {
        ...
        Node endNode = new Node(), childNode = new Node();
        q.add(root);
        q.add(endNode);
        ...
        while (!q.isEmpty()) {
            ...
            if (node == endNode) {
                ...
                // the next node.
                if (q.peek() != endNode) q.add(childNode);
            }
        }
    }
    ...
    private void _deserializeHelper(String data, Node rootNode) {  
        ...
        Queue<Node> currentLevel = new LinkedList<>(), prevLevel = new LinkedList<>();
        ...
        for (int i = 1; i < data.length(); i++) {
            ...
            if (d == '#') {
                ...
                if (d == '$') parentNode = prevLevel.poll();
                else {
